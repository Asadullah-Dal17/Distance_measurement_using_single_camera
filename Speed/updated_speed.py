...
def get_incoming_danger(gui):
    ...
    while True:
        ...
        for (face_x, face_y, face_w, face_h, face_id) in Faces:
            ...
            cv2.putText(
                frame, f"{face_id} Speed: {round(averageSpeed[face_id], 2)} m/s", (30, 50 * face_id), fonts, 0.5, BLACK,
                2)
            ...
            if face_id in speedMap and face_id in distances:
                yield distances[face_id], averageSpeed[face_id], frame
            break
        if gui:
